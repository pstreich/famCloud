            ...
            selection.each(function(data) {
                ...
                data.forEach(function(d){
                    d.axes.forEach(function(axis, i) {
                        let x = getHorizontalPosition(i, cfg.w/2, (parseFloat(Math.max(axis.value, 0))/maxValue)*cfg.factor);
                        let y = getVerticalPosition(i, cfg.h/2, (parseFloat(Math.max(axis.value, 0))/maxValue)*cfg.factor);
                        axis.x = isNaN(x) === true ? 0 : x;
                        axis.y = isNaN(y) === true ? 0 : y;
                    });
                });
                ...
                if(cfg.circles && cfg.radius) {
                    ...
                    circle.enter().append('circle')
                        ...
                        .on('mouseover', function(d){
                            ...
                            var y = d[0].y - 5;
                            tooltip
                                .attr('x',  x.isNaN() === true ? 0 : x)
                                .attr('y', y.isNaN() === true ? 0 : y)
                                .text(d[0].value)
                                .classed('visible', 1);
                    ...
                    circle
                        ...
                        .attr('r', cfg.radius)
                        .attr('cx', function(d) {
                            return isNaN(d[0].x) === true ? 0 : d[0].x;
                        })
                        .attr('cy', function(d) {
                            return isNaN(d[0].y) === true ? 0 : d[0].y;
                        })
                        .each('start', function() {
                            d3.select(this).classed('d3-enter', 0); // trigger css transition
                        });
